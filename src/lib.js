...
export async function postTicketEmbed(ticket) {
  ...
  await fetch(WEBHOOK_TICKETS, {
    ...
    body: JSON.stringify({
      embeds: [{
        title: `Neues Ticket #${ticket.id.slice(0, 6)}`,
        color: 0xd4af37,
        description: `**${ticket.userTag}**\n${lines}\n\n**Summe:** ${ticket.total.toFixed(2)} €`,
...
export async function postProofImage(ticket, file) {
  ...
  form.append('payload_json', JSON.stringify({
    content: `Zahlungsbeweis · Ticket #${ticket.id.slice(0, 6)} · ${ticket.userTag} · ${ticket.total.toFixed(2)} €`,
  }));
...
export async function postTranscript(ticket, messages) {
  ...
  const form = new FormData();
  form.append('payload_json', JSON.stringify({
    content: `Ticket #${ticket.id.slice(0, 6)} archiviert · ${ticket.userTag} · ${ticket.total.toFixed(2)} €`,
  }));
...
function renderTranscript(t, messages) {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><style>
    ...
    <p>Kunde: ${esc(t.userTag)} · Status: ${esc(t.status)}</p>
    <table>${items}<tr><td><b>Summe</b></td><td><b>${t.total.toFixed(2)} €</b></td></tr></table>
    <p class="eyebrow">Zahlung</p><p>${esc(t.payment?.method || '—')} · ${esc(t.payment?.status || '—')}${t.payment?.txHash ? ' · tx ' + esc(t.payment.txHash) : ''}</p>
    <p class="eyebrow">Verlauf</p>${rows || '<p>Keine Nachrichten.</p>'}</body></html>`;
}
